    ...
    suspend fun requestPermissions(permissions: List<Permission>): Boolean {
        return suspendCoroutine { continuation ->
            try {
                ...
                val activity = binding.activity
                val listener = object : RequestPermissionsResultListener {
...
private val PermissionManager.Permission.values
    get() = when (this) {
        dev.hebei.camerax_android.core.PermissionManager.Permission.ALBUM -> if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) emptyArray()
        else arrayOf(
            Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE
        )

        dev.hebei.camerax_android.core.PermissionManager.Permission.AUDIO -> arrayOf(Manifest.permission.RECORD_AUDIO)
        dev.hebei.camerax_android.core.PermissionManager.Permission.VIDEO -> arrayOf(Manifest.permission.CAMERA)
    }

private val List<PermissionManager.Permission>.values: Array<String>
