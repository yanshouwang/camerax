...
public class RotationProvider: NSObject {
    ...
    private lazy var orientationListener = OrientationEventListener() { orientation in
        ...
        for listener in self.listeners {
            guard let sr = try? CameraOrientationUtil.degreesToSurfaceRotation(rotation) else { return }
            listener.onRotationChanged(sr)
        }
    }
    ...
    public func getRotation() throws -> Surface.Rotation {
        return try CameraOrientationUtil.degreesToSurfaceRotation(rotation)
    }
    ...
    public protocol Listener: NSObjectProtocol {
        func onRotationChanged(_ rotation: Surface.Rotation) -> Void
    }
}
